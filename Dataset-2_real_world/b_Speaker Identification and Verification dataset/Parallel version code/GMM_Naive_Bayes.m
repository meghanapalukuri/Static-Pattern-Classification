function [w,myu,Cov]=GMM_Naive_Bayes(cluster_ind,N1,myu1,k,Data,Toler)
...
parfor q=1:k
    vari=cov(Mean_scaled_Cluster_Data(1:N1(q),:,q));
    Cov1(:,:,q)=diag(diag(vari));
end
...
for n=1:length(cluster_ind)
    ...
    parfor q=1:k
        ...
        term1= term1+ w1(q)*mvnpdf(Data_n,myu1(q,:),Cov_q);
    end
    L1= L1+ log(term1);
end
...
for n=1:length(cluster_ind)
    ...
    parfor q=1:k
        Cov_q=Cov(:,:,q);
        gamma(n,q)=w(q)*mvnpdf(Data_n,myu(q,:),Cov_q);
...
while(abs(L-Lold)>Toler)
    ...
    for n=1:length(cluster_ind)
        ...
        parfor q=1:k
            Cov_q=Cov(:,:,q);
            % [~,err(n,q)] = cholcov(Cov_q,0);
            term= term+ w(q)*mvnpdf(Data_n,myu(q,:),Cov_q); % Write as product of normals
        end
        L= L+log(term);
    end
    
    %  abs(L-Lold)
    %% Write code to remove cluster if number of points in cluster is 1
end
end